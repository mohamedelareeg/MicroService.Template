using BuildingBlocks.AppMetaData;
...
public class AuthService : IAuthService
{
    ...
    public async Task<Result<LoginDto>> Login(string email, string password)
    {
        ...
        if (user == null)
        {
            return Result.Failure<LoginDto>(new Error("UserNotFound", "User not found."));
        }
        ...
        if (!result.Succeeded)
        {
            return Result.Failure<LoginDto>(new Error("InvalidCredentials", "Invalid credentials."));
        }
    ...
    public async Task<Result<RegisterDto>> Register(string firstName, string lastName, string email, string userName, string password)
    {
        ApplicationUser? existingUser = await _userManager.FindByNameAsync(userName);

        if (existingUser != null)
        {
            return Result.Failure<RegisterDto>(new Error("UsernameExists", $"Username '{userName}' already exists."));
        }
        ...
        if (existingEmail == null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return Result.Success(new RegisterDto { UserId = user.Id });
            }
            else
            {
                var errors = result.Errors.Select(a => a.Description).ToList();
                return Result.Failure<RegisterDto>(new Error("BadRequestDetails", string.Join(", ", errors)));
            }
        }
        else
        {
            return Result.Failure<RegisterDto>(new Error("EmailExists", $"Email '{email}' already exists."));
        }
    }
